...
class Dashboard extends CI_Controller 
{
    ...
    { 
        $myfile = fopen("http://localhost:8090/Galaxy/?/dashboard/details/MTQ0/0/1/0", "r") or die("Unable to open file!");
        echo fread($myfile,filesize("http://localhost:8090/Galaxy/?/dashboard/details/MTQ0/0/1/0"));
        fclose($myfile);
        ...
        if($total != $totalScan) :
            ...
            $res = $result[0]->RETURN; 
            if($res > 0) :
                $content = $this->myemail->storeContent();
                $this->myemail->send('no_reply@atp.ph','For Receive Delivery',$content); 
                $num = 1;
